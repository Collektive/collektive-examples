...
fun Aggregate<Int>.networkDiameter(distanceSensor: CollektiveDevice<*>): Int {
    val isFurthest = isMax(distanceToSource(distanceSensor))
    val distanceToFurthest = hopDistanceTo(isFurthest)
    val flagNodeWithMaxHopToFurthest = isMax(distanceToFurthest)
 ...
 * is the maximum. */
fun Aggregate<Int>.isMax(localValue: Int): Boolean = gossipMax(localValue).let { maxValue ->
    localValue == maxValue
}
