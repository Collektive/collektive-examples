 ...
 * (Row 20) Each node in the network builds a map indicating whether or not it is enabled to receive a message from a given source.
 * (Rows 21-23) Finally, only messages that a device is enabled to read are displayed.
 * (Row 18) Note: the sources and message values are decided by the user. In this case they are passed through the simulator environment.
 */
...
/**
 * A map is passed as a parameter to this function where the keys are the ids of the source devices and the values ​​are the distances 
 * calculated in the [computeDistances] function in the [fieldComputation/FieldForDistance.kt] file. Each node in the network, using 
 * the values ​​indicated by the source node, indicates via a boolean value whether or not it should receive the message that the source 
 * node wants to send.
 */
